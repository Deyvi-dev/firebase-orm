...
import type { IFirebaseOrmEntity, Constructor, ValidatorOptions } from '../types';

type BatchOperation<T extends IFirebaseOrmEntity> = {
...
export class FirestoreBatchUnit {
  ...
  public operations: BatchOperation<IFirebaseOrmEntity>[] = [];

  constructor(private firestoreRef: Firestore) {}

  add<T extends IFirebaseOrmEntity>(
  ...
  async validate(
    item: IFirebaseOrmEntity,
    Entity: Constructor<IFirebaseOrmEntity>,
    validatorOptions?: ValidatorOptions
    ...
    try {
      ...
      if (error.code === 'MODULE_NOT_FOUND') {
        throw new Error(
          'It looks like class-validator is not installed. Please run `npm i -S class-validator` to fix this error, or initialize Firebase-ORM with `validateModels: false` to disable validation. For more information, please refer to the Firebase-ORM documentation.'
        );
      }
