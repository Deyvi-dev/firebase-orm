...
import {
  IFirebaseOrmEntity,
  IFirebaseOrmQueryBuilder,
  IWherePropParam,
  IFirestoreVal,
  IFirestoreQueryLine,
  IOrderByParams,
  IFirebaseOrmRepository,
  PartialBy,
  IEntityConstructor,
...
export abstract class AbstractFirestoreRepository<T extends IFirebaseOrmEntity>
  extends BaseRepository
  implements IFirebaseOrmRepository<T>
{
   ...
   * @returns {IFirebaseOrmQueryBuilder<T>} A new QueryBuilder with the specified
   * query applied.
   ...
   * @returns {IFirebaseOrmQueryBuilder<T>} QueryBuilder A new QueryBuilder with
  ...
  async validate(item: T): Promise<ValidationError[]> {
    try {
      ...
      if (error.code === 'MODULE_NOT_FOUND') {
        throw new Error(
          'It looks like class-validator is not installed. Please run `npm i -S class-validator` to fix this error, or initialize Firebase-ORM with `validateModels: false` to disable validation.'
        );
      }
   ...
   *
   * @abstract
   * @param {IFirestoreQueryLine[]} queries list of queries stored in
   * QueryBuilder
